...
for nTime = 1:turn
    ...
    for i = 0:N-1   % Update Acceleration
        for j = 1:N
            ...
            if j ~= 1     % left end
                [dx1, dy1, r1] = distClc(x, y, xPos(nTime+1, N*i+j-1), yPos(nTime+1, N*i+j-1));
                [E_pot1, a1] = LJpotential(r1);
            end
            if j ~= N     % right end
                [dx2, dy2, r2] = distClc(x, y, xPos(nTime+1, N*i+j+1), yPos(nTime+1, N*i+j+1));
                [E_pot2, a2] = LJpotential(r2);
            end
            if i ~= 0     % up end
                [dx3, dy3, r3] = distClc(x, y, xPos(nTime+1, N*(i-1)+j), yPos(nTime+1, N*(i-1)+j));
                [E_pot3, a3] = LJpotential(r3);
            end
            if i ~= N-1   % down end
                [dx4, dy4, r4] = distClc(x, y, xPos(nTime+1, N*(i+1)+j), yPos(nTime+1, N*(i+1)+j));
